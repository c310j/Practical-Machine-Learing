...
```
For fast and accurate training of the model, I choose XGBoost, an implementation of tree-based extreme gradient boosting algorithm. 
...
The raw training data has 19622 rows of observations and 158 features (predictors). Column `X` is a unusable row number. While the testing data has 20 rows and the same 158 features. There is one column of target outcome named `classe`.   
...
A conversion of the outcome to numeric is required because XGBoost gradient booster only works with numeric data.   
```{r}
...
zero.var
```
There are no features without variability (everything has enough variance). So there is no feature to be removed further.  
...
A good set of features is when they are highly uncorrelated (orthogonal) to each other. The plot below shows that the average correlation is not too high, so I do not perform further PCA preprocessing.   
...
A tSNE (t-Distributed Stochastic Neighbor Embedding) visualization is a 2D plot of multidimensional features which is a multidimensional reduction into a 2D plane. In the tSNE plot below there is no clear separation of clustering of the 5 levels of outcome (A, B, C, D, E). So it hardly gets conclusion for manually building any regression equation from the irregularity.   
...
Here I build a machine learning model to predict the activity quality (`classe` outcome) from the activity monitors (the features or predictors) by using the XGBoost extreme gradient boosting algorithm.    

#### XGBoost data

XGBoost supports only numeric matrix data. Convert all training, testing and outcome data to matrix.  
...
Expected error rate is less than `1%` for a good classification. Perform cross validation to estimate the error rate using 4-fold cross validation, with 200 epochs to reach the expected error rate of less than `1%`.  
...
Elapsed time is around 318 seconds (5.3 minutes).  

```{r}
tail(bst.cv$dt) 
```
   
From the cross validation, choose the index with minimum multiclass error rate.  
...
```
Best cross-validation's minimum error rate `test.merror.mean` is around 0.006 (0.6%), happened at 187th iteration.   
...
The confusion matrix shows concentration of correct predictions is on the diagonal, as expected.  
...
```
Time elapsed is around 95.5 seconds.  
...
(*The prediction result `pred.char` is not displayed intentionally because it is the answer of the "project submission" part.*)   
...
Feature importance plot is useful to select only best features with the highest correlation to the outcome(s). To improve the model fitting performance (time or overfitting), less important features can be removed.   
